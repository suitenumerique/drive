...
In `interop/data/keycloak/realm.json`, find the `drive` client's `redirectUris` and `webOrigins` arrays and add your IP:
...
- Delete the Keycloak database and recreate both services:
  ...
  ```
- Or add the URIs manually via the Keycloak admin console at `http://<YOUR_IP>:8004/admin/` (admin / admin) → **Clients → drive → Valid redirect URIs**
...
keycloak:
  command:
    ...
    - --import-realm
    - --proxy=edge
    - --hostname=http://<YOUR_IP>:8004 # was {settings.MEDIA_BASE_URL}
...
frontend-dev:
  build:
    args:
      API_ORIGIN: "http://<YOUR_IP>:8200" # was http://localhost:8200
```
...
NEXT_PUBLIC_API_ORIGIN=http://<YOUR_IP>:8200
```
...
```env
OIDC_OP_URL=http://<YOUR_IP>:8004/realms/drive
OIDC_OP_AUTHORIZATION_ENDPOINT=http://<YOUR_IP>:8004/realms/drive/protocol/openid-connect/auth
LOGIN_REDIRECT_URL=http://<YOUR_IP>:8203
LOGIN_REDIRECT_URL_FAILURE=http://<YOUR_IP>:8203
LOGOUT_REDIRECT_URL=http://<YOUR_IP>:8203
OIDC_REDIRECT_ALLOWED_HOSTS="localhost:8004,localhost:8203,<YOUR_IP>:8203"
CSRF_TRUSTED_ORIGINS=http://<YOUR_IP>:8203,http://<YOUR_IP>:8200
MEDIA_BASE_URL=http://<YOUR_IP>:8004
```
