...
test.describe("Release Note", () => {
  ...
  test.describe("Feature flag", () => {
    test("should display release note modal when FRONTEND_HIDE_RELEASE_NOTE is false", async ({
      page,
    }) => {
      await page.route("**/api/v1.0/config/", async (route) => {
        const response = await route.fetch();
        const json = await response.json();
        json.FRONTEND_HIDE_RELEASE_NOTE = false;
      ...
      await page.goto("/");
      await expect(page.getByText("Mise à jours de Fichiers")).toBeVisible();
    });

    test("should NOT display release note modal when FRONTEND_HIDE_RELEASE_NOTE is true", async ({
      page,
    }) => {
      await page.route("**/api/v1.0/config/", async (route) => {
        const response = await route.fetch();
        const json = await response.json();
        json.FRONTEND_HIDE_RELEASE_NOTE = true;
        await route.fulfill({ response, json });
      });
      ...
      await expect(
        page.getByText("Mise à jours de Fichiers"),
      ).not.toBeVisible();
    });
  });
