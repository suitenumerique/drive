...
test.describe("Upload reservations with real object storage", () => {
  ...
  test("reserves before PUT and finalizes without double counting", async ({
    ...
    try {
      ...
      expect(response.request().postDataJSON().size).toBe(1023);
      const { id, policy, size } = await response.json();
      expect(size).toBe(1023);
      ...
      const repeated = await post(page, `${API}/items/${id}/upload-ended/`);
      expect(repeated.status()).toBe(400);
      expect((await repeated.json()).errors[0].code).toBe(
        "item_upload_state_not_pending",
      );
    } finally {
      release();
    }
