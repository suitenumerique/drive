...
async function saveCheckpoint(): Promise<void> {
  ...
  try {
    ...
    // The .odt captures the RENDERED document for persistence and
    // external interop (LibreOffice, Word, download). It does NOT
    // preserve OO's internal object ids — those are regenerated by
    // the load-phase counter on every fresh open. ID alignment across
    // live peers is instead handled by the peer-to-peer state transfer
    // in OOEditor (baseBin + chain), not by anything stored in S3.
    // S3 remains the cold-start source when no peer is around to
    // stream live state.
