...
PROFILES = {
    ...
    "3xl": (200, 20000, 25000, 20, 20, 100, 50, 3),
}
...
class Benchmark:  # pylint: disable=too-many-instance-attributes,too-many-arguments
    ...
    def __init__(  # noqa: PLR0913
        self, *, mode, profile, reps, restrictions, output, commit=None, progress=None
    ):
        self.mode = mode
        ...
        self.output = output
        self.commit = commit
        # Callable receiving one progress line per measured scenario, e.g. the
        # management command's stdout writer; the service never writes by itself.
    ...
    def run_pghero(self, data, result):
        ...
        def run_flat(name, role, url, params):
            """Measure an endpoint responding with a plain list instead of a page."""
            ...
            def request():
                ...
                rows = response.json()
                require(rows, f"Empty benchmark scenario: {name}")
                return tuple(row["id"] for row in rows), len(rows)
        ...
        # The breadcrumb walks the ancestors up to the highest readable one
        # through readable_per_se, on the deepest item of the first chain.
        deepest = models.Item.objects.get(title=f"sub-0000-{self.subtree_depth - 1}")
        run_flat("breadcrumb_bob", "bob", f"/api/v1.0/items/{deepest.pk}/breadcrumb/", {})
        run(
            "shared_roots",
            "reader",
            "/api/v1.0/items/",
