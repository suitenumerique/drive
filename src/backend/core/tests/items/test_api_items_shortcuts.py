"""Tests for the shortcut target contract in the items API."""
...
def test_api_items_shortcuts_children_list_exposes_target():
    """The children listing exposes the shortcut target, greyed for excluded users."""
    ...
    payload = results[str(shortcut.id)]
...
def test_api_items_shortcuts_children_list_target_accessible():
    ...
    assert response.status_code == 200
    results = {result["id"]: result for result in response.json()["results"]}
    assert results[str(folder.shortcut.id)]["target"]["can_access"] is True


def test_api_items_shortcuts_children_list_target_accessible_via_link():
    """A public link reach on the target grants access through the shortcut."""
    parent_owner = factories.UserFactory()
    owner = factories.UserFactory()
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
...
def test_api_items_shortcuts_retrieve_exposes_target():
    """Retrieving a shortcut exposes its target."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        users=[(user, "owner")],
    ...
    response = client.get(f"/api/v1.0/items/{shortcut.id!s}/")

    assert response.status_code == 200
    assert response.json()["target"]["id"] == str(folder.id)


def test_api_items_shortcuts_non_shortcut_target_is_none():
...
def test_api_items_shortcuts_tree_includes_shortcuts():
    ...
    assert str(shortcut.id) in children_ids
