...
def test_api_items_unlock_anonymous_success():
    ...
    assert client.session["unlocked_link_items"] == [item.link_unlock_key]
...
def test_api_items_unlock_inherited_from_ancestor():
    ...
    assert response.status_code == 200
    assert client.session["unlocked_link_items"] == [parent.link_unlock_key]
...
def test_api_items_unlock_password_change_revokes_anonymous_session():
    """Changing the password should revoke an anonymous session's previous unlock."""
    item = factories.ItemFactory(link_reach="public", link_role="reader")
    ...
    assert response.status_code == 200
    assert client.get(f"/api/v1.0/items/{item.id}/").status_code == 200

    item.set_link_password("new-password")
    item.save()

    assert client.get(f"/api/v1.0/items/{item.id}/").status_code == 401

    response = client.post(
        f"/api/v1.0/items/{item.id}/unlock/", {"password": "new-password"}, format="json"
    )
    assert response.status_code == 200
    assert client.get(f"/api/v1.0/items/{item.id}/").status_code == 200
