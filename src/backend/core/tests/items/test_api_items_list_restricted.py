...
def test_api_items_list_restricted_hidden_when_restriction_reachable():
    """A member reaching the live restriction does not see the restricted root."""
...
def test_api_items_list_restricted_follows_moved_restriction():
    """Moving the restriction out of reach makes the restricted root visible again."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        users=[(user, "owner")],
...
def test_api_items_list_restricted_visible_when_restriction_deleted():
    """Without a live restriction the restricted root shows up for its members."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        users=[(user, "owner")],
...
def test_api_items_list_restricted_visible_when_restriction_trashed():
    """A restriction under a soft deleted ancestor does not hide the restricted root."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        users=[(user, "owner")],
...
def test_api_items_list_restricted_hidden_via_link_on_container(reach):
    """An open link on the container makes the restriction reachable and hides the root."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        link_reach=reach,
    ...
    folder = _create_restricted_folder(parent, factories.UserFactory())
    factories.UserItemAccessFactory(item=folder, user=user, role="reader")
    models.LinkTrace.objects.create(item=parent, user=user)
...
def test_api_items_list_restricted_hidden_via_inherited_link():
    """A link inherited from a grandparent counts to reach the restriction."""
    user = factories.UserFactory()
...
def test_api_items_list_restricted_visible_when_container_link_not_visited():
    """A link the user never opened does not hide the restricted root."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        link_reach="public",
