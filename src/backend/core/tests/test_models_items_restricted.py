...
def test_models_items_restricted_restrict_creates_restriction():
    """Activating restriction materializes the origin location with a restriction."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(type=models.ItemTypeChoices.FOLDER, users=[(user, "owner")])
    folder = factories.ItemFactory(parent=parent, type=models.ItemTypeChoices.FOLDER)
    ...
    assert str(restriction.path) == f"{parent.id!s}.{restriction.id!s}"
    assert restriction.title == folder.title
    assert restriction.creator == user
...
def test_models_items_restricted_restrict_rejects_non_owner(role):
    """Only owners can restrict a folder."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(type=models.ItemTypeChoices.FOLDER)
    folder = factories.ItemFactory(parent=parent, type=models.ItemTypeChoices.FOLDER)
    if role:
    ...
    with pytest.raises(ValidationError, match="Only owners can restrict a folder"):
        folder.restrict(user)


def test_models_items_restricted_restrict_is_atomic():
    """A failure during restriction leaves the folder untouched."""
    user = factories.UserFactory()
    parent = factories.ItemFactory(type=models.ItemTypeChoices.FOLDER, users=[(user, "owner")])
    folder = factories.ItemFactory(parent=parent, type=models.ItemTypeChoices.FOLDER)
    ...
    with (
        mock.patch.object(models.ItemManager, "create_child", side_effect=RuntimeError),
    ...
    folder.refresh_from_db()
    assert folder.is_restricted is False
    assert folder.link_reach == link_reach
    assert str(folder.path) == f"{parent.id!s}.{folder.id!s}"
    assert not models.ItemAccess.objects.filter(item=folder).exists()
...
def test_models_items_restricted_cuts_role_inheritance():
    ...
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        users=[(parent_user, models.RoleChoices.OWNER), (user, models.RoleChoices.OWNER)],
    )
    folder = factories.ItemFactory(parent=parent, type=models.ItemTypeChoices.FOLDER)
...
def test_models_items_restricted_unrestrict_removes_redundant_access():
    ...
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        users=[(parent_owner, models.RoleChoices.OWNER), (user, models.RoleChoices.OWNER)],
    )
    folder = factories.ItemFactory(parent=parent, type=models.ItemTypeChoices.FOLDER)
    folder = folder.restrict(user)
    ...
    assert folder.get_role(parent_owner) == models.RoleChoices.OWNER
...
def test_models_items_restricted_unrestrict_keeps_superior_access():
    ...
    parent = factories.ItemFactory(
        type=models.ItemTypeChoices.FOLDER,
        users=[(reader, models.RoleChoices.READER), (user, models.RoleChoices.OWNER)],
    )
    folder = factories.ItemFactory(parent=parent, type=models.ItemTypeChoices.FOLDER)
    folder = folder.restrict(user)
