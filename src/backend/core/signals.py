...
from . import models
from .tasks.search import trigger_file_indexer
...
def document_post_save(sender, instance, **kwargs):  # pylint: disable=unused-argument
    ...
    error.
    """
    transaction.on_commit(partial(trigger_file_indexer, instance))
...
def document_access_post_save(sender, instance, created, **kwargs):  # pylint: disable=unused-argument
    ...
    """
    if not created:
        transaction.on_commit(partial(trigger_file_indexer, instance.item))
