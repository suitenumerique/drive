...
class ItemViewSet(
    ...
    def _create_item(self, serializer, parent=None):
        ...
        if data["type"] == models.ItemTypeChoices.FILE and not extension:
            ...
            if extension:
                permission = get_entitlements_backend().can_upload(self.request.user)
                if not permission["result"]:
                    raise drf.exceptions.PermissionDenied(
                        detail=permission.get("message", "You cannot create files."),
    ...
    def upload_ended(self, request, *args, **kwargs):
        ...
        if item.expected_size is not None and item.upload_state in (
            models.ItemUploadStateChoices.ANALYZING,
            models.ItemUploadStateChoices.READY,
        ):
            return drf_response.Response(self.get_serializer(item).data)
    ...
    def _validate_uploaded_size(self, item, file_size):
        ...
        if item.expected_size is not None and file_size != item.expected_size:
            self._complete_item_deletion(item)
            raise drf.exceptions.ValidationError(
